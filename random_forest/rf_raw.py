...
MODEL_FILE = BASE_DIR / "rf_pipeline_raw.joblib"
PLOT_DIR = BASE_DIR
...
def save_feature_importances(pipeline, suffix=""):
    try:
        ...
        feature_names = [f"f_{i}" for i in range(len(pipeline.named_steps["model"].regressor_.feature_importances_))]
    ...
    if HYPERPARAM_SEARCH:
        ...
        for idx, params in enumerate(HYPERPARAM_GRID, start=1):
            ...
            metrics = evaluate_pipeline(
                f"RF search {idx}/{total} params={params}",
                pipeline,
                X,
                y,
                X_train,
