...
MODEL_FILE = BASE_DIR / "ridge_pipeline_raw.joblib"
PLOT_DIR = BASE_DIR

HYPERPARAM_SEARCH = True
HYPERPARAM_GRID = [
    {"alpha": 0.01}, {"alpha": 0.1}, {"alpha": 0.5},
    {"alpha": 1.0}, {"alpha": 2.0}, {"alpha": 5.0},
    {"alpha": 10.0}, {"alpha": 20.0}, {"alpha": 50.0},
]
...
def save_feature_importances(pipeline, suffix=""):
    try:
        feature_names = pipeline.named_steps["preprocess"].get_feature_names_out()
    except Exception:
        feature_names = [f"f_{i}" for i in range(len(pipeline.named_steps["model"].regressor_.coef_))]

    importances = np.abs(pipeline.named_steps["model"].regressor_.coef_)
...
def evaluate_pipeline(name, pipeline, X, y, X_train, X_test, y_train, y_test, cv, suffix="", save_model_path=None, save_artifacts=True):
    print(f"\n{name}")
    ...
    if save_artifacts:
        ...
        save_feature_importances(pipeline, suffix=suffix)
...
def main():
    ...
    if HYPERPARAM_SEARCH:
        ...
        for idx, params in enumerate(HYPERPARAM_GRID, start=1):
            pipeline = build_model_pipeline(preprocessor, model_params=params)
            metrics = evaluate_pipeline(
                f"RIDGE search {idx}/{total} params={params}",
                pipeline,
                X,
                y,
                X_train,
    ...
    print("\nSummary metrics:")
    print(f"raw: MAE={metrics['mae']:.2f}, RMSE={metrics['rmse']:.2f}, R2={metrics['r2']:.3f}, RMSLE={metrics['rmsle']:.3f}")
    print(f"\nTotal time: {time.time() - start:.1f}s")
