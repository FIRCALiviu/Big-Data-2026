...
DATA_FILE = BASE_DIR.parent / "Datasets" / "umap_dataset.csv"
MODEL_FILE = BASE_DIR / "ridge_pipeline_umap.joblib"
PLOT_DIR = BASE_DIR
...
def main():
	...
	if HYPERPARAM_SEARCH:
		...
		grid_params = [{f"{prefix}alpha": [params["alpha"]]} for params in HYPERPARAM_GRID]
		search = GridSearchCV(
			build_estimator(),
			grid_params,
		...
		evaluate_pipeline(
			f"UMAP best params={best_params}",
			pipeline,
			X,
			y,
			X_train,
