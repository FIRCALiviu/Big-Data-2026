...
def prepare_dataframe(df):
    df = df.copy()
    # basic numeric parsing used in the scraper
    for col in ["surface_m2", "year_built", "number_bathrooms", "latitude", "longitude", "metro_proximity", "stb_proximity"]:
...
def build_model_pipeline(preprocessor, use_pca=False, pca_components=0.95):
...
def save_feature_importances(pipeline, use_pca=False, suffix=""):
    try:
        if use_pca:
    ...
    plt.title("Top Feature Importances" if not use_pca else "Top PCA Component Importances")
    plt.tight_layout()
    suffix = f"_{suffix}" if suffix else ""
...
def evaluate_pipeline(name, pipeline, X, y, X_train, X_test, y_train, y_test, cv, suffix="", save_model_path=None, use_pca=False):
    print(f"\n{name}")
    ...
    plots = save_plots(y_test, preds, suffix=suffix)
    fi_plot = save_feature_importances(pipeline, use_pca=use_pca, suffix=suffix)
